...
namespace BrightstarDB.Client
{
    ...
    public class EmbeddedBrightstarService : IBrightstarService
    {
        ...
        /// </remarks>
        public IJobInfo StartExport(string store, string fileName, string graphUri = null, RdfFormat format = null)
        {
            try
            {
                if (format == null) format = RdfFormat.NQuads.WithEncoding(Encoding.UTF8);
                var jobId = _serverCore.Export(store, fileName, graphUri, format);
                return new JobInfoObject(jobId);
            }
            catch (Exception ex)
            {
