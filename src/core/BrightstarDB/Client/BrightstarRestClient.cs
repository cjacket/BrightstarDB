...
namespace BrightstarDB.Client
{
    ...
    public class BrightstarRestClient : IBrightstarService
    {
        ...
        /// </remarks>
        public IJobInfo StartExport(string store, string fileName, string graphUri, RdfFormat format = null)
        {
            ValidateStoreName(store);
            if (format == null) format = RdfFormat.NQuads.WithEncoding(Encoding.UTF8);
            var job = JobRequestObject.CreateExportJob(fileName, graphUri, format);
            var jobUri = CreateJob(store, job);
            var jobInfoResponse = AuthenticatedGet(jobUri);
            return Deserialize<JobResponseModel>(jobInfoResponse);
        }
